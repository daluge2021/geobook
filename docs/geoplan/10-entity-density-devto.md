...
*DEV.to 发布适配版。使用方式：复制整个文件内容，粘贴到 dev.to/new 编辑器；frontmatter 中的 canonical_url 保证转载不伤原站权重。*

```markdown
---
title: "Entity Association Density: The GEO Metric That Decides Whether AI Cites You"
published: false
description: "Entity Association Density is the GEO metric that predicts whether AI cites your content. 86% of AI citations come from brand-managed sources. Here's how to raise it."
tags: [geo, seo, ai]
canonical_url: "https://geo010.com/metrics/entity-density.html"
cover_image: ""
---
```

> 注：DEV.to 规则——tags 最多 4 个、全小写、可用连字符；`canonical_url` 必须为完整 http(s) URL；`published: false` 存草稿，发布时改为 true 或直接在编辑器点 Publish。
...
*This article draws on the full breakdown at [GEO Encyclopedia — Entity Association Density & Content Cross-Verification](https://geo010.com/metrics/entity-density.html), an open English-language knowledge base on Generative Engine Optimization (GEO).*
```
