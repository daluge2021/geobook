...
## 战略调整（2026-08-16，基于 05 文档第十章效果分析）

### 验证结论回顾

两周数据证明：**内容新增 + 内容结构化（FAQPage/定义前置/freshness）是真实抓取上升的驱动；ai.json 类"发现端点"无抓取回报（0 次访问）**；08-11/08-13 高峰实为扫描攻击，不是抓取增长。

### 资源投向调整

| 原计划项 | 调整 | 原因 |
|----------|------|------|
| `/ai/*.json`、`/.well-known/ai.txt` 推广（含在 P1 llms.txt 增强里） | **降级/不再投入** | 上线 8-04 至 8-16 零访问；AI 爬虫实际入口只有 robots.txt + sitemap.xml |
| 新增内容（后续文章计划） | **提升为最高优先级** | 新文章发布当天即被 GPTBot/ClaudeBot 抓 6-10 次并持续回访，是全站最有效的抓取驱动 |
| 内容结构化改造（存量文章 FAQPage/定义前置/对比表） | **维持高优先级** | 提升"引用质量"而非"抓取数量"，是引用率（06 清单）的上游 |
| freshness（dateModified ↔ lastmod 对齐） | **维持，自动化执行** | 与正常爬虫活跃期吻合，且是 0 成本防御性动作 |
| CSP 等纯安全加固 | **仅防御性保留，不追求抓取回报** | 无抓取正面作用 |

### 下一步行动（下周起）

1. **发布阶段性成果文章**（本文档衍生的英文长文，含 08-04→08-16 真实抓取数据、攻击甄别方法、7 条结论）→ 发布到本站 + 多平台（Medium/LinkedIn/DEV.to/Hashnode/Indie Hackers/HN）。
2. **继续内容新增**：按 01-answer-pages / 03-whitepaper-outline 的待补主题清单推进（优先 AI 可引用的独特概念与数据型文章）。
3. **存量文章结构化改造**：对 06 清单第二/三层对应页面逐页补 FAQPage + 定义前置（沿用 08-16 已完成的 6 页模板）。
4. **每两周复查**：跑本章的"分析过程"四步（提交清单→按天趋势→爬虫/路径细分→剔攻击噪声），验证投入方向是否仍成立。
